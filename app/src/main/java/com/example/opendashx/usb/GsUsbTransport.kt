...
class GsUsbTransport(context: Context, private val recorder: CanSessionRecorder? = null) : CanTransport {
    ...
    @Volatile private var liveFrameListener: ((CanLogFrame) -> Unit)? = null

    var connectionStatus: GsUsbConnectionStatus = GsUsbConnectionStatus()
        private set

    fun setLiveFrameListener(listener: ((CanLogFrame) -> Unit)?) {
    ...
    private fun observeFrame(frame: CanFrame) {
        ...
        stat.observe(frame)
        val logFrame = CanLogFrame(timestampMs = System.currentTimeMillis(), id = frame.id, isExtended = frame.isExtended, dlc = frame.data.size, data = frame.data)
        recorder?.record(logFrame)
        liveFrameListener?.invoke(logFrame)
    }
