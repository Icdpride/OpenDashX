...
class CanLogReplay(private val context: Context) {
    ...
    private fun parseBounded(
        ...
        val capacity = maxFrames.coerceIn(1, 250000)
        val out = ArrayList<CanLogFrame>(capacity.coerceAtMost(60000))
        ...
        file.bufferedReader().useLines { lines ->
            lines.forEach { rawLine ->
                ...
                if (parsed != null) {
                    out.add(parsed)
                    parsedCount++
                    if (out.size > capacity) out.removeAt(0)
                } else {
                    rejectedCount++
                }
                ...
                if (lineCount % 5000 == 0) {
                    ...
                    onProgress(
                        ReplayProgress(
                            ...
                            totalLinesEstimate = estimatedLines,
                            percent = pct,
                            message = "Loaded $parsedCount frames, skipped $rejectedCount rows..."
                        )
                    )
                }
            }
        ...
        onProgress(
            ReplayProgress(
                ...
                totalLinesEstimate = estimatedLines,
                percent = 100,
                message = "Loaded ${out.size} frames, skipped $rejectedCount rows"
            )
        )
        return out
    }
